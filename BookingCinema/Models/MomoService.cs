...
public class MomoService : IMomoService
{
    ...
    public async Task<MomoCreatePaymentResponseModel> CreatePaymentAsync(OrderInfoModel model)
    {
        ...
        if (string.IsNullOrEmpty(_options.Value.PartnerCode) ||
            ...
            string.IsNullOrEmpty(_options.Value.MomoApiUrl) ||
            string.IsNullOrEmpty(_options.Value.RequestType) ||
            string.IsNullOrEmpty(_options.Value.ReturnUrl) ||
            string.IsNullOrEmpty(_options.Value.NotifyUrl))
        {
            throw new InvalidOperationException("Momo service configuration is incomplete.");
        }
        ...
        {
            throw new InvalidOperationException("Momo service response is invalid.");
        }
    ...
    {
        if (collection == null || 
            !collection.ContainsKey("amount") || 
            !collection.ContainsKey("orderInfo") || 
            !collection.ContainsKey("orderId"))
        {
            throw new ArgumentException("Query collection is invalid.");
        }
